...
//从1 到 n 中1出现的次数
//1到13中包含1的数字有1、10、11、12、13因此共出现6次
public class CountAppearanceOf1InInteger {
    ...
    /**
     * 主要思路：
     * 1.设定整数点（如1、10、100等等）作为位置点i（对应n的各位、十位、百位等等），分别对每个数位上有多少包含1的点进行分析
     * 2.根据设定的整数位置，对目标整数n进行分割，分为两部分，高位n/i，低位n%i
     * 3.当i表示百位，且百位对应的数>=2,如n=31456,i=100，则a=314,b=56，此时百位为1的次数有a/10+1=32（最高两位0~31），
     * 每一次都包含100个连续的点，即共有(a/10+1)*100个点的百位为1
     * 4.当i表示百位，且百位对应的数为1，如n=31156,i=100，则a=311,b=56，此时百位对应的就是1，
